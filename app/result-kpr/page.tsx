...
function Metric({ label, value, detail, icon: Icon }: { label: string; value: string; detail?: string; icon?: typeof WalletCards }) {
  return (
    ...
    <main className="min-h-screen bg-[#f7f9fb] pb-24 font-poppins text-slate-900">
      <header className="border-b border-slate-200 bg-white px-5 py-4 sm:px-8">
        <div className="mx-auto flex max-w-360 items-center justify-between gap-6">
          <Image src="/jakloan-logo.png" alt="Jakloan" width={260} height={80} className="h-8 w-auto object-contain sm:h-10" />
          ...
          <Button className="h-9 rounded-md bg-[#ef2b1f] px-4 text-xs font-semibold hover:bg-[#d92117]">Ajukan KPR</Button>
        </div>
      </header>
      ...
      <section className="relative isolate overflow-hidden border-b border-slate-200 bg-white">
        <div className="mx-auto grid max-w-360 items-center gap-5 px-5 pt-8 sm:px-8 lg:grid-cols-[1.05fr_0.95fr] lg:pt-10">
          <div className="relative z-10 pb-8 lg:pb-12">
            <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.2em] text-[#ef2b1f]">KPR Griya Monas</p>
            <h1 className="max-w-xl text-3xl font-bold tracking-tight text-slate-950 sm:text-5xl">Simulation for Every Plan</h1>
            <p className="mt-3 max-w-lg text-xs leading-5 text-slate-500 sm:text-sm">
              Simulasi KPR Griya Monas untuk membantu Anda merencanakan pembiayaan rumah dengan lebih percaya diri.
            </p>
          ...
          <div className="relative hidden h-48 lg:block lg:h-60">
            <Image src="/building.png" alt="Modern home" fill className="object-contain object-bottom-right" priority />
          </div>
        </div>
      </section>
        ...
        <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-200 px-5 py-4">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <ReadOnlyField label="Penghasilan Bulanan (Anda)" value="15.000.000" />
              <ReadOnlyField label="Penghasilan Bulanan (Pasangan)" value="7.000.000" />
              <ReadOnlyField label="Cicilan / Utang Berjalan per Bulan" value="2.000.000" />
              <ReadOnlyField label="Status Pekerjaan" value="Karyawan Swasta" select />
            </div>
            <div className="mt-4 grid grid-cols-2 gap-2 rounded-md bg-slate-50 p-3 sm:grid-cols-4">
              ...
              <MiniStat label="Sisa Kapasitas Cicilan" value="Rp5.333.333" detail="(24,2%)" />
            </div>
          ...
          <SectionRow index={2} title="Properti" icon={Home} open={openSection === 2} onClick={() => toggleSection(2)} />
          {openSection === 2 && (
          ...
          <SectionRow index={3} title="Pembiayaan" icon={WalletCards} open={openSection === 3} onClick={() => toggleSection(3)} />
          {openSection === 3 && (
            <div className="grid gap-3 bg-slate-50 px-5 py-4 sm:grid-cols-2">
          ...
          <SectionRow index={4} title="Suku Bunga" icon={TrendingUp} open={openSection === 4} onClick={() => toggleSection(4)} />
          {openSection === 4 && (
            <div className="grid gap-3 bg-slate-50 px-5 py-4 sm:grid-cols-2">
              <ReadOnlyField label="Jenis Suku Bunga" value="Fixed → Floating" />
        ...
        <section className="space-y-3">
          <div className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
            ...
            <div className="grid grid-cols-2 divide-x divide-y divide-slate-200 sm:grid-cols-4 sm:divide-y-0">
              <Metric label="Estimasi Cicilan Bulanan" value="Rp4.873.000" detail="Fixed 3 Tahun" icon={CircleHelp} />
              <Metric label="Cicilan Setelah Floating" value="Rp5.612.000" detail="Floating Tahun ke-4" icon={CircleHelp} />
              <Metric label="Jumlah Pinjaman" value="Rp900.000.000" icon={Info} />
          ...
          <div className="grid gap-3 sm:grid-cols-[1.05fr_1fr]">
            <div className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
              <p className="text-[11px] font-bold">Financial Fit Indicator (Simulasi)</p>
              <div className="mt-3 flex items-center gap-4">
                ...
                <div>
                  <p className="text-xs font-bold text-emerald-600">Lebih Sesuai</p>
            ...
            <div className="grid grid-cols-2 gap-3">
              <SummaryCard label="DSR (Total)" value="31,2%" detail="Acuan simulasi ≤ 33%" accent />
              <SummaryCard label="LTV" value="90%" detail="Batas maks. simulasi: 90%" accent />
            </div>
          </div>

          <div className="grid gap-3 sm:grid-cols-3">
            <SummaryCard label="Dana Awal" value="Rp108.850.000" detail="5 Komponen" />
            <SummaryCard label="Total Bunga (Estimasi)" value="Rp478.509.000" detail="Selama tenor" />
            <SummaryCard label="Status" value="Layak" detail="Dengan catatan" accent />
          </div>
